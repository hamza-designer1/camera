...
void ACaptureManager::updateResolution(int newFrameW, int newFrameH)
{
    ...
    GEngine->AddOnScreenDebugMessage(-10, 1.f, FColor::Yellow, FString::Printf(TEXT("New Dimension: %d"), FrameWidth));
    ...
    if (!RenderRequestQueue.IsEmpty()) {
        ...
        if (nextRenderRequest) { //nullptr check
            if (nextRenderRequest->RenderFence.IsFenceComplete()) { // Check if rendering is done, indicated by RenderFence
                ...
                if (UsePNG) {
                    //Generate image name
                    fileName = FPaths::ProjectSavedDir()+"data/"+ SubDirectoryName + "/img" + "_" + ToStringWithLeadingZeros(ImgCounter, NumDigits);
